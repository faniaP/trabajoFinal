...
%% necesitamos agregar el número de jugadores con los que decidimos acotarlo%%%%
%% tambien necesitamos agregar lo de las tropas cuando reforamos los paises%%%

\section{Descripci\'on del agente.}
Aqu\'i es donde describimos el comportamiento del agente.
%%% tipo de agente del proyecto%%%
%%% sería un agente basado en modelo%%%
...
%%%% Este  hay que modificarlo y dice fernando que el lo 
...
%%%%generacion de los posibles movimientos%%%%%
...
\FORALL{$\nu$ $\in$ $V_{jugador}$}
\IF{$\nu$ $tiene VecinoVacio()$}
\STATE{$v.avanza(g.node)$}
\RETURN{$g'$}
\ELSE 
\STATE {$posiblesMovimientos(Jugador 2, g)$}
\ENDIF
\ENDFOR
\end{algorithmic}
